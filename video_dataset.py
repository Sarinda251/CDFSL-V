...
class VideoRecord(object):
    ...
    Args:
        root_datapath: the system path to the root folder
                       of the videos.
        row: A list with four or more elements where 0) The first
             element is the path to the video sample's frames excluding
             the root_datapath prefix 1) The  second element is the starting frame id of the video
             1) The third element is the inclusive ending frame id of the video
             2) The fourth element is the label index.
             3) any following elements are labels in the case of multi-label classification
    """
    def __init__(self, row, root_datapath):
        self._data = row
        self._path = os.path.join(root_datapath, row[0])
        #print(os.path.join(root_datapath, row[0]))
